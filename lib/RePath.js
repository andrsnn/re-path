...
var RePath = function (root, extension, defaultFile) {
	this.root = root;
	this.extension = extension || '.js';
	this.defaultFile = defaultFile || 'index' + this.extension;
};
				...
				self.findPathsInFile(
					...
					function (matchingLine, absPathToCurrentFile, callback) {
						...
						var resolvedFilePath = self.utils.resolvePathFromFile(absPathToCurrentFile, lineMatch),
							possiblePaths = self.getPossiblePaths(resolvedFilePath);

						self.areFilesAtPaths(possiblePaths, function (err, fileExists) {
							if (err) {
								return callback(err);
							}
							if (!fileExists) {
